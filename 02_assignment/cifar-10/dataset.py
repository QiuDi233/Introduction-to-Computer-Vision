...
class CIFAR10(torch.utils.data.Dataset):
    ...
    def __init__(self, train=True):
        ...
        self.data = np.vstack(self.data).reshape(-1, 3, 32, 32) #若干张3x32x32的图片 （cifar10是32x32的RGB）
    ...
    def __getitem__(self, index):
        ...
        """
        img, target = self.data[index], self.targets[index] # img <class 'numpy.ndarray'> (32, 32, 3)
        ...
        img = img.astype(np.float32) #<class 'numpy.ndarray'> 
...
if __name__ == '__main__':
    ...
    # --------------TODO------------------
    # 对图片进行颜色变换
    color_transform = transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5)
    aug2 = color_transform(img)
