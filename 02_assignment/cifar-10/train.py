...
def My_log_softmax(pred):
    #输入是torch.Size([20, 10])
    ex_pred = torch.exp(pred)
    ex_sum = torch.sum(ex_pred,axis = 1)
    pred_softmax = ex_pred/ex_sum.view(-1,1)
    pred_log_softmax = torch.log(pred_softmax)
    return pred_log_softmax


def MyCELoss(pred, gt): #torch.Size([20, 10]) torch.Size([20]) 20是batch-size 
    # ----------TODO------------
    # Implement CE loss here
    batch_size, num_classes = pred.shape #20,10

    # 先算softmax 将预测数据转换为概率
    # 库函数 用于debug
    #log_probs = torch.nn.functional.log_softmax(pred, dim=1)
    # 自己写的log_softmax
    log_probs = My_log_softmax(pred)
    
    #对每个batch 是gt的地方放1 不是的放0
    target = torch.zeros(batch_size, num_classes).to(gt.device)
    target.scatter_(1, gt.view(-1, 1), 1)

    loss = -(target * log_probs).sum(dim=1).mean()
    # ----------TODO------------
    #库函数 用于debug
    #loss1 = F.cross_entropy(pred, gt)
    #print(loss1)
    #print(loss)
